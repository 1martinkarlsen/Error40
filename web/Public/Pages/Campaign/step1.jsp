    ...
    <c:forEach var="campaign" items="${sessionScope.allCampaigns}">
        <c:if test="${sessionScope.cID == campaign.value['id']}">
            <form action="Servlet?origin=updateCampaign" method="post">
                ...
                <p>Start date</p>
                <select name="start_day" id="start_day" value="<c:out value="${campaign.value['start_day']}" />">
            ...
            <div class="checkedApprove">
                <div class="checkedSeller">
                    <c:if test="${campaign.value['approve_seller_project'] == 0}">
                        <img src="Public/images/waiting.png" id="imgSeller">
                    </c:if>
                    <c:if test="${campaign.value['approve_seller_project'] == 1}">
                        <img src="Public/images/approved.png" id="imgSeller">
                    </c:if>
                    <c:if test="${campaign.value['approve_seller_project'] == 2}">
                        <img src="Public/images/declined.png" id="imgSeller">
                    </c:if>
                    <p>Seller</p>
                </div>
                <div class="checkedPartner">
                    <c:if test="${campaign.value['approve_partner_project'] == 0}">
                        <img src="Public/images/waiting.png" id="imgSeller">
                    </c:if>
                    <c:if test="${campaign.value['approve_partner_project'] == 1}">
                        <img src="Public/images/approved.png" id="imgSeller">
                    </c:if>
                    <c:if test="${campaign.value['approve_partner_project'] == 2}">
                        <img src="Public/images/declined.png" id="imgSeller">
                    </c:if>
                    <p>Partner</p>
                </div>
            ...
            <div class="approve_buttons">
                <c:if test="${campaign.value['stepNumber'] == 1}">
                    <c:if test="${sessionScope.user.id == campaign.value['sellerID']}">
                        <div class="but_approve"><a href="Servlet?origin=approveCampaignRequest&choice=1">Approve</a></div>
                        <div class="but_decline"><a href="Servlet?origin=approveCampaignRequest&choice=2">Decline</a></div>
                    </c:if>

                    <c:if test="${sessionScope.user.id == campaign.value['partnerID']}">
                        <div class="but_approve"><a href="Servlet?origin=approveCampaignRequest&choice=1">Approve</a></div>
                        <div class="but_decline"><a href="Servlet?origin=approveCampaignRequest&choice=2">Decline</a></div>
                    </c:if>
                </c:if>
